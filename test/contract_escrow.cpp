...
class EscrowChecker : public RANDOM
{
public:
...
public:
    ...
    {
        return (EscrowChecker*)contractStates[RANDOM_CONTRACT_INDEX];
    }

    void beginEpoch(bool expectSuccess = true)
    {
        callSystemProcedure(RANDOM_CONTRACT_INDEX, BEGIN_EPOCH, expectSuccess);
    }

    void endEpoch(bool expectSuccess = true)
    {
        callSystemProcedure(RANDOM_CONTRACT_INDEX, END_EPOCH, expectSuccess);
    }

    void createDeal(const id& caller, const RANDOM::CreateDeal_input& input, const int64_t& quAmount)
    {
        RANDOM::CreateDeal_output output;
        invokeUserProcedure(RANDOM_CONTRACT_INDEX, 1, input, output, caller, quAmount);
    }

    void operateDeal(const id& caller, const int64_t& dealIndex, const EscrowOperation type, const int64_t& quAmount)
    {
        RANDOM::AcceptDeal_input input{ dealIndex };
        RANDOM::AcceptDeal_output output;
        invokeUserProcedure(RANDOM_CONTRACT_INDEX, static_cast<unsigned short>(type), input, output, caller, quAmount);
    }

    RANDOM::GetDeals_output getDeals(const id& user, const int64_t& privateDealsOffset, const int64_t& publicDealsOffset)
    {
        RANDOM::GetDeals_input input{ user, privateDealsOffset, publicDealsOffset };
        RANDOM::GetDeals_output output;
...
TEST(ContractEscrow, CreateDeal)
{
    ...
    int ownershipIndex;
    int possessionIndex;
    issueAsset(testAddress1, "TOKEN", 0, "000000", 1000000, RANDOM_CONTRACT_INDEX, &issuanceIndex, &ownershipIndex, &possessionIndex);

    RANDOM::AssetWithAmount emptyAsset = { {NULL_ID}, 0, 0 };

    // scenario 1: attempt to create a deal by testAddress1, but not enough commissions
    RANDOM::CreateDeal_input input{};
    ...
    RANDOM::GetDeals_output testAddress1Deals = escrow.getDeals(testAddress1, 0, 0);
    ...
    RANDOM::GetDeals_output testAddress2Deals = escrow.getDeals(testAddress2, 0, 0);
...
TEST(ContractEscrow, OperateDeal)
{
    ...
    int ownershipIndex;
    int possessionIndex;
    issueAsset(testAddress2, "TOKEN", 0, "000000", 1000000, RANDOM_CONTRACT_INDEX, &issuanceIndex, &ownershipIndex, &possessionIndex);
    ...
    EXPECT_EQ(testAddress1Deals.ownedDeals.get(0).acceptorId, id(RANDOM_CONTRACT_INDEX, 0, 0, 0));
    ...
    issueAsset(NULL_ID, "RANDOM", 0, "000000", 676, QX_CONTRACT_INDEX, &issuanceIndex, &ownershipIndex, &possessionIndex);
    int destinationOwnershipIndex;
    int destinationPossessionIndex;
    // transfer 10 RANDOM shares to shareHolder address  
    ...
    // attempt to accept deal, not enough qu for call
    escrow.operateDeal(testAddress2, 2, EscrowOperation::AcceptDeal, 1);
    EXPECT_EQ(escrow.getState()->getDealsAmount(), 1);

    // accept deal, success
    escrow.operateDeal(testAddress2, 2, EscrowOperation::AcceptDeal, 250000);
    EXPECT_EQ(escrow.getState()->getDealsAmount(), 0);

    EXPECT_EQ(numberOfPossessedShares(assetNameFromString("TOKEN"), testAddress2, testAddress1, testAddress1, RANDOM_CONTRACT_INDEX, RANDOM_CONTRACT_INDEX), 490000);
    EXPECT_EQ(numberOfPossessedShares(assetNameFromString("TOKEN"), testAddress2, id(RANDOM_CONTRACT_INDEX, 0, 0, 0), id(RANDOM_CONTRACT_INDEX, 0, 0, 0), RANDOM_CONTRACT_INDEX, RANDOM_CONTRACT_INDEX), 10000);  // 2% commission
    
    // token distribution to shareholders
    escrow.endEpoch();
    // shareHolder address has 10 RANDOM shares and should receive: 10000 TOKEN / 676 shares = 14 TOKEN per share, and 14 TOKEN per share * 10 shares = 140 TOKEN
    EXPECT_EQ(numberOfPossessedShares(assetNameFromString("TOKEN"), testAddress2, shareHolder, shareHolder, RANDOM_CONTRACT_INDEX, RANDOM_CONTRACT_INDEX), 140);
